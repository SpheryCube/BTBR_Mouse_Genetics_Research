...
# triple.fit(gene.exp("Gprc5b", liver.rz), log(clinical("INS.10wk", phenotypes)), genotype(chr=3, pos=49.5))
#Hypothesis: 

#We are looking at the genotype of insrr
triple.fit(gene.exp("Insrr", gastroc.rz), clinical("INS.10wk", phenotypes), genotype(chr=3, pos=38.70)) 

triple.fit(gene.exp("Insrr", gastroc.rz), clinical("IL.6", phenotypes), genotype(chr=3, pos=38.70))

triple.fit(gene.exp("Insrr", gastroc.rz), clinical("HOMA.10wk", phenotypes), genotype(chr=3, pos=38.70))
