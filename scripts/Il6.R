...
summary(subset(scan_cond, chr = 5))[,which(names(scan_cond) %in% c("Thap6", "Cmklr1"))]
...
f2g$pheno <- cbind(f2g$pheno[,c("MouseNum","Sex","pgm")], phenotypes.rz[,c("INS.10wk", "IL.6")], Il6_adipose, Thap6_adipose, Cmklr1_adipose)
# look at all pairwise scatterplots of clinical and expression traits
names(f2g$pheno)
par(mfrow=c(1,1))
pairs(f2g$pheno[,4:length(f2g$pheno)], upper.panel=panel.cor,diag.panel=panel.hist)

f2g$pheno <- cbind(f2g$pheno[,c("MouseNum","Sex","pgm")], phenotypes.rz[,c("INS.10wk", "IL.6")], Il6_adipose, Thap6_adipose, Cmklr1_adipose, Chst1_adipose)



scan2 <- scanone(f2g,  pheno.col = c("Thap6_adipose", "Cmklr1_adipose", "Chst1_adipose"), method = "hk", addcovar = sex)
# Identify LOD significance thresholds for gene expression traits.
...
#Thap6_adipose at 51.8
#Cmklr1_adipose at 54.7
...
print("BIC Analysis with Insulin and IL-6.")


#Thap at 51.8 on 5 according to scan2.
