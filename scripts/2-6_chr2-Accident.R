...
chr2_genes_small <- scan(file = "/home/daniel14/Il6_Mouse_Research/data/ch2_genes(small).txt",  what = "character",   skip = 1)
...
f2g$pheno <- cbind(f2g$pheno,  adipose.rz[,match(chr2_genes_small,annot$gene1, nomatch = 0)])
names(f2g$pheno)
...
# significance threshold of 3.00 only for insulin?
...
# peaks on chromosome 5 for all three scans - insulin, Atg5, Atg7.


summary_obj <- summary(subset(scan_cond, chr = 5))[,which(names(scan_cond) %in% candidate_genes)]
summary_obj

#This code looks at which genes bring the lod score below the user defined lod_threshold for insulin, Il6, and Il6 adipose expression.
x = 0
lod_threshold = 3.67
while (3*x < length(summary_obj))
{
  scores = vector()
  for (lod_score in summary_obj[(1+3*x):(3+3*x)])
  {
