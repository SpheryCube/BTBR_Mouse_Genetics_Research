...
myQTLnet_function <- function(tissue, phenotype, ...) {
  #print(tissue)
  print(phenotype)


  f2g$pheno <- cbind(f2g$pheno[,c("MouseNum","Sex","pgm")],phenotypes.rz[c(toString(phenotype))])
  ...
  #define variables
  for (gene_name in input_list)
  {
    gene <- tissue[,annot$a_gene_id[which(annot$gene_symbol==toString(gene_name))]]
    print(toString(gene))
    f2g$pheno <- cbind(gene)
  }
...
myQTLnet_function(islet.rz, "INS.10wk", Insrr, Il6)
