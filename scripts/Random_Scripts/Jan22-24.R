# Hypothesis: Increased IL6 levels will cause a decrease in Insr gene expression, thereby preventing glucose metabolism and leading to diabetes.
# Model: 
# increased IL6 levels -> decreased Insr gene expression ->higher blood glucose levels
# This code is modeled after atg3.R
...
###################################################################
# Find the insulin receptor gene (gene symbol Insr) in the annotation data.
grep("Insr", annot$gene1)
...
annot[grep("Insr", annot$gene1),]
annot[grep("Insr", annot$gene1), 1]

# Retrieve Insr expression data from the adipose tissue.
...
f2g$pheno <- cbind(f2g$pheno[,c("MouseNum","Sex","pgm")],phenotypes.rz[,c("GLU.10wk","INS.10wk", "IL.6")],  insrr_adipose, insrr_gastroc) #Used to include "TRIG.10wk","HOMA.10wk",
# look at all pairwise scatterplots of clinical and expression traits

names(f2g$pheno)
X11()
pairs(f2g$pheno[,4:length(f2g$pheno)])
# There is no strong correlation between any of the clinical phenotypes and gene expression data.
...
scan1 <- scanone(f2g,  pheno.col = c("GLU.10wk","INS.10wk", "insrr_adipose", "insrr_gastroc"), method = "hk", addcovar = sex)
...
# Plot genome scans for insr.
# Add a title and LOD thresholds.
  par(mfrow=c(2,1)) # Plots in 2 rows, one column. Stands for partition.
  plot(scan1, lodcolumn = 3, main = "Quantitative Trait Loci for Insrr Expression in adipose tissue")
  ...
  plot(scan1, lodcolumn = 4, main = "Quantitative Trait Loci for Insrr Expression in gastronomic tissue")
  add.threshold(scan1, perms=perm1, alpha=0.05,lty="dashed",lwd=2,col="orange")
  add.threshold(scan1, perms=perm1, alpha=0.10,lty="dashed",lwd=2,col="purple")

  # # Plot genome scans for Atg5, Atg7, and insulin. Add a title and LOD thresholds.
  # par(mfrow=c(3,1)) # Plots in 3 rows, 1 column.
  # plot(scan1, lodcolumn = 2, main = "Quantitative Trait Loci for  Insulin at 10 Weeks")
  # add.threshold(scan1, perms=perm1, alpha=0.05,lty="dashed",lwd=2,col="orange")
  # add.threshold(scan1, perms=perm1, alpha=0.10,lty="dashed",lwd=2,col="purple")
  # 
  # plot(scan1, lodcolumn = 10, main = "Quantitative Trait Loci for Atg5 Expression in Pancreatic Islet")
  # add.threshold(scan1,perms=perm1, alpha=0.05,lty="dashed",lwd=2,col="orange")
  # add.threshold(scan1, perms=perm1, alpha=0.10,lty="dashed",lwd=2,col="purple")
  # 
  # plot(scan1, lodcolumn = 11, main = "Quantitative Trait Loci for  Atg7 Expression in Pancreatic Islet")
  # add.threshold(scan1, perms=perm1, alpha=0.05,lty="dashed",lwd=2,col="orange")
  # add.threshold(scan1,perms=perm1, alpha=0.10,lty="dashed",lwd=2,col="purple")
...
# Jan-22 to Jan-24 script usage ends here
#Below is uncharted territory
...
scan_atg5 <- scanone(f2g, pheno.col="INS.10wk", addcovar=f2g$pheno$atg5_islet, method="hk")
scan_atg7 <- scanone(f2g, pheno.col="INS.10wk", addcovar=f2g$pheno$atg7_islet, method="hk")
...
plot(scan_atg5, main = "Insulin Scan with Islet Atg5 Expression as Covariate",ylab = "INS.10wk")
add.threshold(scan1, perms=perm1, alpha=0.05, lty="dashed", lwd=2, col="orange")
add.threshold(scan1,perms=perm1, alpha=0.10, lty="dashed", lwd=2, col="purple")

plot(scan_atg7,main = "Insulin Scan with Islet Atg7 Expression as Covariate",ylab = "INS.10wk")
add.threshold(scan1,perms=perm1, alpha=0.05,lty="dashed", lwd=2, col="orange")
add.threshold(scan1,perms=perm1, alpha=0.10,lty="dashed", lwd=2, col="purple")
...
f2g$pheno <- cbind(f2g$pheno,  islet.rz[,match(chr2_genes,annot$gene1, nomatch = 0)])
names(f2g$pheno)
...
scan_cond <- scanone(f2g,  pheno.col=c("INS.10wk", "atg5_islet", "atg7_islet"), addcovar=f2g$pheno$Crnkl1, method="hk")
